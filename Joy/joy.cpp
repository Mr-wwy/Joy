...
void Joy::fun()
{
//    diagnostic_.add("Joy Driver Status", this, &Joy::diagnostics);
//    diagnostic_.setHardwareID("none");

    // Parameters
    ros::NodeHandle nh_param("~");
    pub_ = nh_.advertise<sensor_msgs::Joy>("joy", 1);
//    ros::Subscriber sub = nh_.subscribe("joy/set_feedback", 10, &Joy::set_feedback, this);
    nh_param.param<std::string>("dev", joy_dev_, "/dev/input/js0");
//    nh_param.param<std::string>("dev_ff", joy_def_ff_, "/dev/input/by-id/usb-Sony_PLAYSTATION_R_3_Controller-event-Joy");
    ...
    if (!joy_dev_name_.empty())
    {
        ...
        {
            ROS_ERROR("Couldn't find a Joy with name %s. Falling back to default device.", joy_dev_name_.c_str());
        }
        else
        {
            ROS_INFO("Using %s as Joy device.", joy_dev_path.c_str());
...
//      diagnostic_.force_update();
        ...
        while (true)
        {
            ...
            if (joy_fd != -1)
            {
              ...
              // Joy are not the values of the Joy when it was opened
              // but rather the values of the Joy when it was last closed.
            ...
            if (first_fault)
            {
                ROS_ERROR("Couldn't open Joy %s. Will retry every second.", joy_dev_.c_str());
                first_fault = false;
            }
            sleep(1.0);
//          diagnostic_.update();
...
/*
        ...
        // Set the gain of the device
            ...
            if (write(ff_fd_, &ie, sizeof(ie)) == -1)
                ROS_ERROR("Couldn't open Joy force feedback!");//perror("set gain");
...
*/
        ROS_INFO("Opened Joy: %s. deadzone_: %f.", joy_dev_.c_str(), deadzone_);
        open_ = true;
//      diagnostic_.force_update();
        ...
        while (nh_.ok())
        {
            ...
            FD_ZERO(&set); //将指定的文件描述符集清空，在对文件描述符集合进行设置前(select函数)，必须对其进行初始化，
                //如果不清空，由于在系统分配内存空间后，通常并不作清空处理，所以结果是不可知的
            FD_SET(joy_fd, &set); //将joy_fd添加到set中

            //ROS_INFO("Select...");
            int select_out = select(joy_fd+1, &set, NULL, NULL, &tv); //测试指定的fd可读
            //ROS_INFO("Tick...");
            if (select_out == -1)
            {
                tv.tv_sec = 0;
...
*/
            if (FD_ISSET(joy_fd, &set)) //用于测试指定的文件描述符joy_fd是否在set集合中
            {
                ...
                switch(event.type)
                {
                    ...
                    case JS_EVENT_BUTTON | JS_EVENT_INIT:
                        ...
                        joy_msg.buttons[event.number] = (event.value ? 1 : 0); //按键消息赋值给ros消息结构体
                        ...
                        else
                        {
                            if (!(event.type & JS_EVENT_INIT))
                            {
                                ...
                                if(val > unscaled_deadzone)
                                    ...
                                    val=0;
                                joy_msg.axes[event.number]= val * scale; //按键消息赋值给ros消息结构体
                            }
...
//      close(ff_fd_);
        ...
        if (nh_.ok())
        {
            ROS_ERROR("Connection to Joy device lost unexpectedly. Will reopen.");
        }
    }
...
void Joy::findDevice()//找到设备
{
    ...
    while (getline(_device, _str))//读一行数据
    {
        ...
        else
        {
            ...
            if (pos != std::string::npos)
            {
                _str = _str.substr(pos + 9, _str.size());
                pos = _str.find(" ");
                if (pos != std::string::npos)
                {
                    _str = _str.substr(0, pos);
                    joy_dev_ = "/dev/input/";
                    joy_dev_.append(_str.c_str());
                    ROS_INFO("event name: [%s]", joy_dev_.c_str());
                    break;;
                }
            }
        }
...
/*
void Joy::set_feedback(const sensor_msgs::JoyFeedbackArray::ConstPtr& msg)
{
    if (ff_fd_ == -1)
...
*/
/*
void Joy::diagnostics(diagnostic_updater::DiagnosticStatusWrapper& stat)
{
    ...
    stat.add("recent Joy event rate (Hz)", event_count_ / interval);
...
std::string Joy::get_dev_by_joy_name(const std::string& joy_name)
{
    ...
    {
      // filter entries
      if (strncmp(entry->d_name, "js", 2) != 0) // skip device if it's not a Joy
        continue;
      ...
      ROS_INFO("Found Joy: %s (%s).", current_joy_name, current_path.c_str());
